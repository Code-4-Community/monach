...
import { PickRequired, Narrow, ReadonlyDeep, RequiredKeys, UndefinedIfNever, DeepReadonlyObject } from './utils.types.js';
import z from 'zod';
...
declare class ZodiosClass<Api extends ZodiosEndpointDefinitions> {
     ...
     */
    request<M extends Method, Path extends ZodiosPathsByMethod<Api, M>, TConfig = ReadonlyDeep<ZodiosRequestOptions<Api, M, Path>>>(config: TConfig): Promise<ZodiosResponseByPath<Api, M, Path>>;
    /**
...
declare class ZodiosError extends Error {
    readonly config?: DeepReadonlyObject<{
        ...
        paramsSerializer?: axios.ParamsSerializerOptions | undefined;
 ...
 */
declare function isErrorFromPath<Api extends ZodiosEndpointDefinitions, M extends Method, Path extends ZodiosPathsByMethod<Api, M>>(api: Api, method: M, path: Path, error: unknown): error is ZodiosMatchingErrorsByPath<Api, M, Path>;
/**
 * check if the error is matching the endpoint errors definitions
 * @param api - the api definition
 ...
 * @returns - if true, the error type is narrowed to the matching endpoint errors
 */
declare function isErrorFromAlias<Api extends ZodiosEndpointDefinitions, Alias extends Aliases<Api>>(api: Api, alias: Alias, error: unknown): error is ZodiosMatchingErrorsByAlias<Api, Alias>;
 ...
 * @returns - the api definitions
 */
declare function makeCrudApi<T extends string, S extends z.ZodObject<any>>(resource: T, schema: S): [{
    method: "get";
    path: `/${T}s`;
...
export { ApiOf, PluginId, Zodios, ZodiosClass, ZodiosConstructor, ZodiosError, ZodiosInstance, apiBuilder, checkApi, formDataPlugin, formURLPlugin, headerPlugin, isErrorFromAlias, isErrorFromPath, makeApi, makeCrudApi, makeEndpoint, makeErrors, makeParameters, zodValidationPlugin };
