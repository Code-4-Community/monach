...
type ZodiosSucessCodes = 200 | 201 | 202 | 203 | 204 | 205 | 206 | 207 | 208 | 226;
...
interface ZodiosRequestHandler<Api extends ZodiosEndpointDefinitions, Context extends ZodObject<any>, M extends Method, Path extends ZodiosPathsByMethod<Api, M>, ReqPath = ZodiosPathParamsByPath<Api, M, Path, false>, ReqBody = ZodiosBodyByPath<Api, M, Path, false>, ReqQuery = ZodiosQueryParamsByPath<Api, M, Path, false>, Res = ZodiosResponseByPath<Api, M, Path, false>> {
    (req: WithZodiosContext<express.Request<ReqPath, Res, ReqBody, ReqQuery>, Context>, res: Omit<express.Response<Res>, "status"> & {
        status<StatusCode extends number, API extends ZodiosEndpointDefinition[] = Api, METHOD extends Method = M, PATH extends ZodiosPathsByMethod<Api, M> = Path>(status: StatusCode): StatusCode extends ZodiosSucessCodes ? express.Response<Res> : express.Response<ZodiosErrorByPath<API, METHOD, PATH, StatusCode, false>>;
    }, next: express.NextFunction): void;
}
interface ZodiosRouterContextRequestHandler<Context extends ZodObject<any>> {
...
type ZodiosApp<Api extends ZodiosEndpointDefinitions, Context extends ZodObject<any>> = IfEquals<Api, any, Omit<ReturnType<typeof express>, "use"> & ZodiosUse<Context>, Omit<ReturnType<typeof express>, Method | "use"> & ZodiosHandlers<Api, Context>>;
...
export { WithZodiosContext, ZodiosApp, ZodiosAppOptions, ZodiosContext, ZodiosHandler, ZodiosHandlers, ZodiosRequestHandler, ZodiosRouter, ZodiosRouterContextRequestHandler, ZodiosRouterOptions, ZodiosSucessCodes, ZodiosUse, ZodiosValidationOptions, zodiosApp, zodiosContext, zodiosNextApp, zodiosRouter };
